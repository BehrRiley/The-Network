    ...
    script:
        ...
        - choose <[Action]>:
            ...
            - case BuyPrompt:
                ...
                - define MeeseeksInfo <item[Action_Item].with[display_name=<&3>M<&b>eeseeks<&sp><&3>I<&b>nfo<&3>:;material=player_head;nbt=<list[Menu/BawbShop|Action/BuyPrompt]>;skull_skin=<server.flag[Behrry.Essentials.SavedHeads].map_get[MeeseeksBox]>]>
            ...
            # $ ██ [ if <player.flag[Behrry.Economy.Coins]> >= <[MeeseeksPrice]>: ] ██
                ...
                # $ ██ [ flag player Behrry.Economy.Coins:-:<[MeeseeksPrice]>     ] ██
                    ...
                    - define Box "<item[Action_Item].with[material=player_head;display_name=<&3>M<&b>eeseeks<&sp><&3>T<&b>ransportation<&sp><&3>B<&b>ox;nbt=<list[Script/SpawnMeeseeks|uniquifier/<util.random.uuid>]>;skull_skin=<server.flag[Behrry.Essentials.SavedHeads].map_get[Package]>;lore=<list[<proc[Colorize].context[<element[ There's an unspawned| Meeseeks in here!].escaped>|green]>]>]>"
...
Meeseeks_Handler:
    ...
    debug: false
    MeeseeksCheck:
        - if <player.has_flag[Behrry.Meeseeks.ActiveFollow]>:
            - foreach <player.flag[Behrry.Meeseeks.ActiveFollow]> as:NpcID:
                - inject locally <[1]> npc:<npc[<[NpcID]>]>
    Despawn:
        - if <npc.is_spawned>:
            - flag <npc> Behrry.Meeseeks.Dedupe duration:5t
            - despawn <npc>
            - wait 1t
        - flag player Behrry.Meeseeks.ActiveFollow:<-:<npc.id>
    Spawn:
    # @ ██ [ Spawn Behind us ] ██
        - wait 1t
        - define Loc <Player.eye_location.add[<player.location.direction.vector.mul[-1]>]>
        - if <npc.is_spawned>:
            - flag <npc> Behrry.Meeseeks.Dedupe duration:5t
            - despawn <npc>
            - wait 1t
        - adjust <npc> spawn:<[Loc]>

    events:
    # @ ██ [ Shop Checks ] ██
    # $ ██ [ on player closes MeeseeksShop_*: ] ██
    # $ ██ [ - note as:<context.inventory.notable_name> remove ] ██
        on player opens inventory:
            - if <server.flag[Meeseeks.Shop.ChestLock].parse[before[/]].contains[<context.inventory.location||null>]>:
                - define ChestData <server.flag[Meeseeks.Shop.ChestLock].map_get[<context.inventory.location>].split[/]>
                - flag <npc[<[ChestData].get[2]>]> Behrry.Meeseeks.ReManage
        on player clicks in MeeseeksShop*:
            - determine passively cancelled
            - define Npc <npc[<context.inventory.after_last[_]>]>
            - define Owner <[Npc].owner>

        # @ ██ [ Owner Actions ] ██
            - if <player> == <[Owner]>:

                #@ Check if editing shop
                - if <[Npc].flag[Behrry.Meeseeks.Mode]> == SetupShop:

                    #@ Check if Multiplying Counter
                    - if <context.click> == number_key:
                        - define Value <element[10].power[<context.hotbar_button.sub[1]>]>
                        - flag player Behrry.Meeseeks.Shop.PriceModifier:<[Value]>

                        - define Lore:|:<context.inventory.slot[<context.inventory.size.sub[4]>].lore.get[1]>
                        - if <[Value]> == 1:
                            - define "Lore:->:<&3>left<&b>-<&3>click<&b>: <&2>+<&a><[Value].format_number.replace[,].with[<&2>,<&a>]> Coin"
                        - else:
                            - define "Lore:->:<&3>left<&b>-<&3>click<&b>: <&2>+<&a><[Value].div[1000].mul[1000].format_number.replace[,].with[<&2>,<&a>]> Coins"
                        - define "Lore:->:<&3>shit<&b>+<&3>left<&b>-<&3>click<&b>: <&2>+<&a><[Value].mul[10].mul[1000].div[1000].format_number.replace[,].with[<&2>,<&a>]> Coins"
                        - define "Lore:->:<&3>right<&b>-<&3>click<&b>: <&4>-<&c><[Value].mul[1000].div[1000].format_number.replace[,].with[<&4>,<&c>]> Coins"
                        - define "Lore:->:<&3>shift<&b>+<&3>right<&b>-<&3>click<&b>: <&4>-<&c><[Value].mul[10].mul[1000].div[1000].format_number.replace[,].with[<&4>,<&c>]> Coins"
                        - define Lore:|:<context.inventory.slot[<context.inventory.size.sub[4]>].lore.get[6].to[11]>
                        - define "Lore:->:<&3><&m>----<&6>[<&b>Current Multiplier:<&6>]<&3><&m>----"
                        - if <[Value]> == 1:
                            - define "Lore:->:<&3>Multiplier<&b>-<&6>[<&e>1<&6>]<&b>: <&2>+<&a><[Value].format_number.replace[,].with[<&2>,<&a>]>x Coin"
                        - else:
                            - define "Lore:->:<&3>Multiplier<&b>-<&6>[<&e><[Value].log[10].round_up.add[1]><&6>]<&b>: <&2>+<&a><[Value].div[1000].mul[1000].format_number.replace[,].with[<&2>,<&a>]><&2>x <&a>Coins"
                        - inventory adjust d:<context.inventory> slot:<context.inventory.size.sub[4]> lore:<[Lore]>
                        - stop

                    #@ Check if clicking in shop area
                    - if !<util.list_numbers_to[<context.inventory.size.sub[9]>].contains[<context.raw_slot>]>:
                        - stop
                    #@ Check if clicking an empty slots
                    - if <context.item.material.name> == air:
                        - stop

                    - define Item <context.item.with[lore=;remove_nbt=Price|InventoryOrigin]>
                    - define ItemPrice <context.item.nbt[Price]>
                    - define Multiplier <player.flag[Behrry.Meeseeks.Shop.PriceModifier]>
                    - choose <context.click>:
                        - case left:
                            - define Price <[ItemPrice].add[<[Multiplier].mul[1]>]>
                        - case shift_left:
                            - define Price <[ItemPrice].add[<[Multiplier].mul[10]>]>
                        - case right:
                            - define Price <[ItemPrice].sub[<[Multiplier].mul[1]>]>
                        - case shift_right:
                            - define Price <[ItemPrice].sub[<[Multiplier].mul[10]>]>
                        - case control_drop:
                            - define Price 1
                        - default:
                            - stop
                    ...
                    #- inventory set d:<context.inventory> slot:<context.raw_slot> o:air
                    - wait 1t
                    ...
                    - flag <[Npc]> Meeseeks.Shop.ItemPrices:<-:<[Item].escaped>/<[ItemPrice]>
                    - flag <[Npc]> Meeseeks.Shop.ItemPrices:->:<[Item].escaped>/<[Price]>
                - else:
                    - if !<player.has_flag[Behrry.Interaction.ViewOnlyNotice]>:
                        - flag player Behrry.Interaction.ViewOnlyNotice duration:10s
                        - narrate format:Colorize_Red "You are in View-Only Mode."
                
        # @ ██ [ Player Actions ] ██
            - else:
                #@ Check if NPC is open for shop
                #- if <[Npc].flag[Behrry.Meeseeks.Mode]> != "OpenShop":
                - if false:
                    #$ To-Do: Settings
                    - narrate format:Colorize_Red "Not Open For Shop"
                    - stop

            # @ ██ [ Check if clicking items in Shop ] ██
                - if <util.list_numbers_to[<context.inventory.size.sub[9]>].contains[<context.raw_slot>]>:
                    - determine passively cancelled
                # @ ██ [ Determine Click-Type ] ██
                    ...
                    #@ Check which Inventory was clicked in
                    - if <context.clicked_inventory||null> == null || <context.clicked_inventory||null> == <player.inventory>:
                        - stop

                    #@ Check if clicking air
                    - if <context.item.material.name> == air:
                        - stop

                # @ ██ [ Check for missing price error ] ██
                    - if !<context.item.has_nbt[Price]>:
                        - announce to_console "<&4><element[].pad_right[69].with[=]>ERROR<element[].pad_right[69].with[=]>"
                        - announce to_console "Missing PRICE"
                        - stop

                # @ ██ [ check for missing origin error ] ██
                    - if !<context.item.has_nbt[InventoryOrigin]>:
                        - announce to_console "<&4><element[].pad_right[69].with[=]>ERROR<element[].pad_right[69].with[=]>"
                        - announce to_console "Missing ORIGIN"
                        - stop

                    - define Price <context.item.nbt[Price]>
                    - define StackQuantity <context.item.quantity>
                ...
                # @ ██ [ Check Player Space ] ██
                    - define StrippedItem <context.item.with[lore=;remove_nbt=Price|InventoryOrigin]>
                    - if <[StackQuantity]> > <[Quantity]>:
                        - define TakeQuantity <[Quantity]>
                        #- inventory remove d:<context.inventory> slot:<context.raw_slot> o:<context.item.with[quantity=<[TakeQuantity]>]>
                        - take from:<context.inventory> slot:<context.raw_slot> quantity:<[TakeQuantity]>
                    - else:
                ...
                # @ ██ [ Check player inventory size ] ██
                    ...
                    - flag <[Owner]> Behrry.Economy.Coins:+:<[TotalPrice]>
                    - take <[StrippedItem].with[quantity=<[TakeQuantity]>]> from:<context.item.nbt[InventoryOrigin].as_location.inventory>
                    #- inventory remove d:<context.item.nbt[InventoryOrigin].as_location.inventory> o:<[StrippedItem].with[quantity=<[TakeQuantity]>]>
                    ...
                    - give <[StrippedItem].with[quantity=<[TakeQuantity]>]>
            # $ ██ [ Add check for back/forward pages ] ██
                - else:
                    - determine cancelled
        on player drags in MeeseeksShop_*:
            - determine cancelled
    # @ ██ [ Spawn Checks ] ██
        on player joins:
            - run locally MeeseeksCheck def:Spawn
        on player respawns:
            - run locally MeeseeksCheck def:Spawn
        on player changes world:
            - run locally MeeseeksCheck def:Spawn
        on player teleports:
            #- announce to_console "<&e><&lt>context.entity<&gt><&a> <context.entity||null> <&3>returns the EntityTag."
            #- announce to_console "<&e><&lt>context.origin<&gt><&a> <context.origin||null> <&3>returns the LocationTag the entity teleported from."
            #- announce to_console "<&e><&lt>context.destination<&gt><&a> <context.destination||null> <&3>returns the LocationTag the entity teleported to."
            #- announce to_console "<&e><&lt>context.cause<&gt><&a> <context.cause||null> <&3>returns an ElementTag of the teleport cause. Can be:"
            - if <context.cause> == unknown:
                #- announce to_console "<&c>-- Stopping, per UNKNOWN cause--"
                - stop
            - run locally MeeseeksCheck def:Spawn

    # @ ██ [ Despawn Checks ] ██
        on player quits:
            - run locally MeeseeksCheck def:Despawn
        on player death:
            - run locally MeeseeksCheck def:Despawn
    ...
    # @ ██ [ Entity Check ] ██
        on npc despawns:
            - define NpcID <npc.id>
            - if <npc.has_flag[Behrry.Meeseeks.Dedupe]>:
                - stop
            - foreach <server.list_online_players_flagged[Behrry.Meeseeks.ActiveFollow]> as:Player:
                - if <[player].flag[Behrry.Meeseeks.ActiveFollow].contains[<[NpcID]>]>:
                    - run locally MeeseeksCheck def:Despawn player:<[Player]> npc:<npc[<[NpcID]>]>
